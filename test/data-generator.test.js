...
describe(`data-generator`, () => {
  describe(`generateEntity`, () => {
    it(`should return generated data`, () => {
      ...
      assert.equal(checkValuesFromRange(data.offer.rooms, 1, 5), true, `Price should return values from range`);
      ...
      assert.equal(checkin.includes(data.offer.checkin), true, `Should contain one o f the value`);
      assert.equal(checkout.includes(data.offer.checkout), true, `Should contain one of the value`);
      assert.equal(features.includes(data.offer.features), true, `Should contain one of the value`);
      assert.equal(photos.includes(data.offer.photos), true, `Should contain one of the value`);
      assert.equal(photos.includes(data.offer.description), '', `Description should be a blank string`);
      assert.equal(checkValuesFromRange(data.location.x, 300, 900), true, `Price should return values from range`);
      assert.equal(checkValuesFromRange(data.location.y, 150, 500), true, `Price should return values from range`);
